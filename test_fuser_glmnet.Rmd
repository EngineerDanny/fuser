...
#beta_fuser = fusedLassoProximal(X, y, groups, lambda=0.01, gamma=0, 
#                                G=G, intercept=FALSE, scaling=FALSE)

# Generate block diagonal matrices for L2 fusion approach
#transformed.data = generateBlockDiagonalMatrices(X, y, groups, G, scaling = F)
...
# Fit fused L2
G = matrix(1, k, k)
beta_fused = fusedL2DescentGLMNet(X_train, y_train, groups_train, G, 
                                  lambda=0.01, gamma=0.1)
pred_fused = predictFusedL2(beta_fused, X_test, groups_test, groups_train)

# Fit standard glmnet
glmnet_model = glmnet(X_train, y_train, alpha=1, lambda=0.01, standardize=FALSE)
pred_glmnet = predict(glmnet_model, newx=X_test, s=0.01)
